...
def train(args):
    ...
    train_file=open(os.path.join(log_directory, method_name, model_name, 'log_train_running.txt'),'w+')
    validation_file=open(os.path.join(log_directory, method_name, model_name, 'log_valid_running.txt'),'w+')
    ...
    for epoch in range(args.num_epochs):
        train_file.write("****************Training epoch beginning******************"+'\n')
        ...
        for batch in range(dataloader.num_batches):
            ...
            epoch_temp=epoch * dataloader.num_batches + batch
            time_temp=end - start
            total_batches_temp=args.num_epochs * dataloader.num_batches
            temp_file_start=dataloader.get_file_name()
            train_file.write("Now processing:"+temp_file_start+'\n')
            train_file.write("Current Batch: "+str(epoch_temp)+" Total Batches:"+ str(total_batches_temp) +" Epoch:" +str(epoch)+" loss: "+str(loss_batch)+"time/batch:"+str(time_temp)+'\n')
        ...
        if dataloader.valid_num_batches > 0:
            validation_file.write("****************Validation epoch beginning******************"+'\n')
            ...
            for batch in range(dataloader.num_batches):
                ...
                if dataset_pointer_ins is not dataloader.dataset_pointer:
                    if dataloader.dataset_pointer is not 0:
                        file_name_temp_2=dataloader.get_file_name(-1)
                        avg_error_1=err_epoch/num_of_batch
                        validation_file.write("Finished processing file:"+file_name_temp_2+"Average error:"+str(avg_error_1)+'\n')
                        print('Finished processing file : ', dataloader.get_file_name(-1),' Average error : ', err_epoch/num_of_batch)
                        num_of_batch = 0
                ...
                for sequence in range(dataloader.batch_size):
                    ...
                    file_name_temp=dataloader.get_file_name(0)
                    time_temp_1=end-start
                    validation_file.write("Current File: "+ file_name_temp+" Batch:"+str(batch+1)+" Sequence:"+str(sequence+1)+" Sequence mean error:"+str(err)+" Sequence final error:"+str(f_err)+" time:"+str(time_temp)+'\n')
